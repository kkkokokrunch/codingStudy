...
## js里的垃圾回收机制
1. 程序运行的过程中会产生垃圾，这些垃圾积攒过多以后会导致程序运行速度过慢
2. 当一个对象没有任何的变量或者属性对他进行引用，此时我们将永远无法操作该对象，这样的对象就是一个垃圾
3. 在JSzhong拥有自动地垃圾回收机制，会自动将这些垃圾对象从内存中销毁，我们不需要也不能进行垃圾回收的操作
4. 我们需要做的只是将不再使用的对象设置为null
